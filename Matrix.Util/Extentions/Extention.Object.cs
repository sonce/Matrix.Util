...
namespace Matrix.Util.Extentions
{
    public static partial class Extention
    {
        ...
        /// <param name="obj">对象</param>
        /// <returns></returns>
        public static T? DeepClone<T>([NotNull] this T obj) where T : class
        {
        ...
        public static string ToXmlStr<T>(this T obj, string? rootNodeName = null)
        {
            ...
            if (jsonStr == null)
                return string.Empty;
            XmlDocument? xmlDoc;
            if (rootNodeName.IsNullOrEmpty())
                xmlDoc = JsonConvert.DeserializeXmlNode(jsonStr);
            else
                xmlDoc = JsonConvert.DeserializeXmlNode(jsonStr, rootNodeName);
            if (xmlDoc == null)
                return string.Empty;
        ...
        /// 获取某属性值
        /// </summary>
        /// <typeparam name="T">属性类型值</typeparam>
        /// <param name="obj">对象</param>
        /// <param name="propertyName">属性名</param>
        /// <returns></returns>
        ...
        /// <summary>
        /// <paramref name="type"/>是否继承至<paramref name="parentType"/>
        /// </summary>
        /// <param name="type">子类</param>
        /// <param name="parentType">父类</param>
        /// <returns></returns>
        public static bool IsInherite(this Type type, Type parentType)
        {
            return parentType.Ge‌​tTypeInfo().IsAssignableFrom(type);
            //return type.IsAssignableFrom(parentType.Ge‌​tTypeInfo());
        }

        /// <summary>
        /// <paramref name="type"/>是否继承至<typeparamref name="T"/>
        /// </summary>
        /// <typeparam name="T">基类</typeparam>
        /// <param name="type">子类型</param>
        /// <returns></returns>
        public static bool IsInherite<T>(this Type type)
        {
            return type.IsInherite(typeof(T));
        }
    }
}
